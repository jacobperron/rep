...
We can have a tool like ``ros2 service echo example_interfaces/srv/AddTwoInts`` that internally subscribes to the two hidden topics ``example_interfaces/srv/AddTwoInts_RequestEvent`` and
``example_interfaces/srv/AddTwoInts_ResponseEvent`` and echoes them. It should also support additional command line parameters used for topics, like ``--filter, --no-arr, --no-str, --include-msg-info``, etc.
