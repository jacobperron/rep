...
Internally, it would subscribe to the hidden topics and echo them. The existing command line parameters for topics can be extended to be used with this ``echo`` verb, along with new
...
Proposed command line usage example
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Suppose we have a service named ``/add_two_ints`` of type ``example_interfaces/srv/AddTwoInts``.
We can extend ``ros2 service`` with an ``echo`` verb such that it internally subscribes to the two hidden topics ``example_interfaces/srv/AddTwoInts_RequestEvent``, ``example_interfaces/srv/AddTwoInts_ResponseEvent`` and echoes them.
